 ...
 * Requests are same-origin, so the Supabase session cookies are sent automatically.
 */
...
export const api = {
  ...
  listAllDoctors: () => apiFetch("/doctors?includeInactive=1"),
  createDoctor: (payload) => apiFetch("/doctors", { method: "POST", body: payload }),
  updateDoctor: (id, payload) => apiFetch(`/doctors/${id}`, { method: "PATCH", body: payload }),
  deactivateDoctor: (id) => apiFetch(`/doctors/${id}`, { method: "DELETE" }),
  listAllServices: () => apiFetch("/services?includeInactive=1"),
  createService: (payload) => apiFetch("/services", { method: "POST", body: payload }),
  updateService: (id, payload) => apiFetch(`/services/${id}`, { method: "PATCH", body: payload }),
  deactivateService: (id) => apiFetch(`/services/${id}`, { method: "DELETE" }),
  // Content management (admin)
  listContent: () => apiFetch("/content"),
  updateContentBlock: (key, payload) => apiFetch(`/content/${encodeURIComponent(key)}`, { method: "PUT", body: payload }),
  resetContentBlock: (key) => apiFetch(`/content/${encodeURIComponent(key)}`, { method: "DELETE" }),
  listSettings: () => apiFetch("/settings"),
  updateSetting: (key, payload) => apiFetch(`/settings/${encodeURIComponent(key)}`, { method: "PUT", body: payload }),
  listAllFaqs: () => apiFetch("/faqs?includeInactive=1"),
  createFaq: (payload) => apiFetch("/faqs", { method: "POST", body: payload }),
  updateFaq: (id, payload) => apiFetch(`/faqs/${id}`, { method: "PATCH", body: payload }),
  deleteFaq: (id) => apiFetch(`/faqs/${id}`, { method: "DELETE" }),
  listAllTestimonials: () => apiFetch("/testimonials?all=1"),
  createTestimonial: (payload) => apiFetch("/testimonials", { method: "POST", body: payload }),
  updateTestimonial: (id, payload) => apiFetch(`/testimonials/${id}`, { method: "PATCH", body: payload }),
