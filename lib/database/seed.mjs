...
// The seed UPSERTS doctors, services, FAQs, schedules and website settings
// from the bundled demo data, which would overwrite edits made in the
// dashboard. It therefore refuses to run in production unless explicitly
// forced (intended only for a brand-new database).
if (isProduction() && !process.argv.includes("--force-production")) {
  console.error("db:seed is disabled with NODE_ENV=production because it overwrites catalogue, FAQ and settings rows with demo content.");
  console.error("Run it only against a fresh database with: node lib/database/seed.mjs --force-production");
  process.exit(1);
}
