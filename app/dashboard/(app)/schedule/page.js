...
export default async function DashboardSchedulePage() {
  ...
  const all = await listDoctors({ includeInactive: scope.all });
  ...
  return (
    <>
      ...
      {doctors.length === 0 ? (
        ...
        <ScheduleManager doctors={doctors.map((d) => ({ id: d.id, name: d.name }))} canManageAll={scope.all} initialDoctorId={doctors[0].id} readOnly={!canManage} />
      )}
    </>
  );
}
